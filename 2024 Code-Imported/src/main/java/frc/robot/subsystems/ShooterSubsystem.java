...
public class ShooterSubsystem extends SubsystemBase {
  ...
  private SparkMaxConfig m_followerMotorConfig = new SparkMaxConfig();
  ...
  public ShooterSubsystem() {
    ...
    m_shooterMotorConfig.signals.faultsPeriodMs(5);
    m_shooterMotorConfig.signals.primaryEncoderVelocityPeriodMs(10);
    m_shooterMotorConfig.signals.primaryEncoderPositionPeriodMs(MessageTimers.Status2);
    m_shooterMotorConfig.signals.analogVoltagePeriodMs(MessageTimers.Status3);
    m_shooterMotorConfig.signals.externalOrAltEncoderPosition(MessageTimers.Status4);
    m_shooterMotorConfig.signals.externalOrAltEncoderVelocity(MessageTimers.Status4);
    // m_shooterMotor.setPeriodicFramePeriod(CANSparkLowLevel.PeriodicFrame.kStatus5,
    // MessageTimers.Status5);
    m_shooterMotorConfig.signals.motorTemperaturePeriodMs(MessageTimers.Status6);
    m_shooterMotor.configure(m_shooterMotorConfig, ResetMode.kResetSafeParameters,
        PersistMode.kPersistParameters);

    // shooter follower CAN messages rates
    m_followerMotorConfig.signals.faultsPeriodMs(MessageTimers.Status0);
    m_followerMotorConfig.signals.primaryEncoderVelocityPeriodMs(MessageTimers.Status1);
    m_followerMotorConfig.signals.primaryEncoderPositionPeriodMs(MessageTimers.Status2);
    m_followerMotorConfig.signals.analogVoltagePeriodMs(MessageTimers.Status3);
    m_followerMotorConfig.signals.externalOrAltEncoderPosition(MessageTimers.Status4);
    m_followerMotorConfig.signals.externalOrAltEncoderVelocity(MessageTimers.Status4);
